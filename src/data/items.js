import satinImage from "../Components/Assests/thirdImage.png"
import peperomia from "../Components/Assests/Peperomia.png"
import catathea from "../Components/Assests/calathea.png"
import monstera from "../Components/Assests/monstera.png"
import ballet from "../Components/Assests/ballet.png"
import ranunculus from "../Components/Assests/ranunculus.png"
import colorful from "../Components/Assests/colorful.png"
import blushing from "../Components/Assests/blushing.png"
export  const data = [
  {
    id: 1,
    name : "Satin Pothos",
    price : 16,
    description : "Introducing the Satin Pothos, a stunning addition to any indoor      space. With its heart-shaped leaves featuring a lush satin-like texture and striking silver variegation, this plant adds a touch of elegance to any room. Thriving in low to medium light conditions, it's an ideal choice for busy plant enthusiasts seeking low-maintenance greenery. Whether adorning a shelf, hanging gracefully in a basket, or cascading from a planter, the Satin Pothos brings a touch of natural beauty to any home or office environment.",
    quantity : 6,
    image: satinImage,
    careTips: "To keep your Satin Pothos thriving, ensure it receives bright, indirect light. Direct sunlight can scorch its delicate leaves. Keep the soil consistently moist but not waterlogged, allowing the top inch to dry out between waterings. Satin Pothos enjoys higher humidity levels, so misting the leaves or placing a humidity tray nearby can help. Additionally, regularly wipe the leaves with a damp cloth to remove dust, which can hinder its ability to photosynthesize. With these care tips, your Satin Pothos will grow beautifully!",
    category: "plant"
  },
  
  {
    id: 4,
    name : "Calathea Medallion",
    price : 29,
    description : "Introducing the Calathea Medallion - a botanical masterpiece for your indoor oasis. With its intricately patterned leaves resembling medallions of artistry, this plant captivates with its elegance and charm. Perfect for adding a touch of tropical flair to any room, the Calathea Medallion thrives in medium to bright indirect light, making it an ideal choice for well-lit living spaces or offices. With its unique ability to fold its leaves at night, it adds an interactive element to your plant collection. Elevate your space with the beauty of the Calathea Medallion, a true gem among houseplants.",
    quantity : 2,
    image: catathea,
    careTips: "To care for your Calathea Medallion, provide it with filtered, indirect sunlight as direct sun can scorch its leaves. Keep the soil consistently moist but not soggy, allowing the top inch to dry out between waterings. Calathea Medallion thrives in high humidity, so misting the leaves or using a pebble tray can help maintain moisture levels. Use lukewarm water when watering to avoid shocking the plant. Additionally, avoid cold drafts and sudden temperature fluctuations as they can stress the plant. With proper care, your Calathea Medallion will display its stunning foliage patterns for years to come!",
    category: "plant"
  },
  {
    id: 5,
    name : "The Monstera",
    price : 34,
    description : "Introducing the Monstera - the epitome of botanical chic. With its iconic Swiss cheese-like leaves and unmistakable tropical charm, this plant makes a bold statement in any space. Thriving in bright, indirect light and with minimal watering needs, the Monstera is the perfect choice for both novice and seasoned plant enthusiasts alike. Whether gracing a corner of your living room or adding a touch of green to your office, its lush foliage and dramatic silhouette instantly elevate any environment. Embrace the allure of the Monstera and bring a slice of the jungle into your home.",
    quantity : 2,
    image: monstera,
    careTips: "The Monstera, known as the Swiss Cheese Plant, thrives in bright, indirect light, avoiding direct sunlight to prevent leaf scorching while ensuring sufficient illumination for optimal growth. Watering should be moderate, allowing the top inch of soil to dry out between waterings to prevent root rot, with higher humidity levels maintained through misting or placing the plant on a pebble tray. Supporting its growth with a moss pole or trellis encourages upright development. Regular pruning of yellowing or damaged leaves promotes overall health and aesthetics. Fertilize with a balanced liquid fertilizer during the growing season, and repot every 1-2 years to prevent overcrowding.",
    category: "plant"
  },
  {
    id: 6,
    name : "The Ballet",
    price : 60,
    description : "Introducing Ballet- a bouquet that dances with delicate grace and timeless charm. Inspired by the soft hues of pink and white roses, this exquisite arrangement evokes the elegance of a prima ballerina on stage. Each bloom whispers of romance and sophistication, creating a captivating ensemble that enchants the senses. Perfect for celebrating love, friendship, or any special occasion, 'Ballet' is a poetic expression of beauty and grace that leaves an enduring impression on all who behold it.",
    quantity : 2,
    image: ballet,
    careTips:"To maintain the beauty of your flower bouquet, start by trimming the stems at an angle and placing them in a clean vase filled with room temperature water. Remember to change the water every 2-3 days, trimming the stems each time to ensure optimal hydration. Keep the bouquet away from direct sunlight, heat sources, and drafts, and consider misting the flowers if the air is dry. Removing any wilted flowers promptly and avoiding submerging leaves in the water will help prevent bacterial growth. With these simple care steps, your flower bouquet will stay fresh and vibrant for longer, brightening up your space and lifting your spirits.",
    category: "flower"
  },
  {
    id: 3,
    name : "Rainbow Ranunculus",
    price : 420,
    description : "Introducing our vibrant and lively 'Rainbow Ranunculus' bouquet - a burst of color to brighten any day. These charming blooms boast a kaleidoscope of hues, ranging from bold reds to soft pinks, sunny yellows, and serene blues, creating a symphony of color that captivates the eye. Each delicate petal unfolds like a tiny work of art, radiating joy and vitality. Whether adorning a special occasion or simply bringing cheer to your home, our Rainbow Ranunculus bouquet is a celebration of life's beauty and diversity.",
    quantity : 12,
    image: ranunculus,
    careTips:"To maintain the beauty of your flower bouquet, start by trimming the stems at an angle and placing them in a clean vase filled with room temperature water. Remember to change the water every 2-3 days, trimming the stems each time to ensure optimal hydration. Keep the bouquet away from direct sunlight, heat sources, and drafts, and consider misting the flowers if the air is dry. Removing any wilted flowers promptly and avoiding submerging leaves in the water will help prevent bacterial growth. With these simple care steps, your flower bouquet will stay fresh and vibrant for longer, brightening up your space and lifting your spirits.",
    category: "flower"
  },
  {
    id: 2,
    name : "Peperomia Watermelon",
    price : 19,
    description : "Meet the Peperomia Watermelon, a delightful addition to your plant collection. Its vibrant green leaves, adorned with striking watermelon-like patterns, bring a playful charm to any space. Compact and easy to care for, this petite plant thrives in bright, indirect light, making it perfect for desks, shelves, or windowsills. With its unique foliage and low-maintenance nature, the Peperomia Watermelon is sure to be a conversation starter and a cherished accent in your home or office.",
    quantity : 7,
    image: peperomia,
    careTips: "To care for your Peperomia Watermelon plant, ensure it receives bright, indirect light, avoiding direct sunlight to prevent leaf damage. Water the plant thoroughly when the top inch of soil feels dry, but be cautious not to overwater as Peperomias are prone to root rot. Maintain moderate to high humidity levels by misting the leaves regularly or using a humidifier, especially in dry environments. Keep the plant in a temperature range of 65-80°F (18-27°C) and provide it with well-draining soil to prevent waterlogging. Fertilize with a balanced, diluted fertilizer every 4-6 weeks during the growing season, and prune any yellowing or damaged leaves to promote healthy growth. With these care practices, your Peperomia Watermelon will thrive and display its vibrant foliage.",
    category: "plant"
  },
  {
    id: 7,
    name : "Colourful pot",
    price : 19,
    description : "Meet the Peperomia Watermelon, a delightful addition to your plant collection. Its vibrant green leaves, adorned with striking watermelon-like patterns, bring a playful charm to any space. Compact and easy to care for, this petite plant thrives in bright, indirect light, making it perfect for desks, shelves, or windowsills. With its unique foliage and low-maintenance nature, the Peperomia Watermelon is sure to be a conversation starter and a cherished accent in your home or office.",
    quantity : 7,
    image: colorful,
    careTips:"To maintain the beauty of your flower bouquet, start by trimming the stems at an angle and placing them in a clean vase filled with room temperature water. Remember to change the water every 2-3 days, trimming the stems each time to ensure optimal hydration. Keep the bouquet away from direct sunlight, heat sources, and drafts, and consider misting the flowers if the air is dry. Removing any wilted flowers promptly and avoiding submerging leaves in the water will help prevent bacterial growth. With these simple care steps, your flower bouquet will stay fresh and vibrant for longer, brightening up your space and lifting your spirits.",
    category: "flower"
  },
  {
    id: 8,
    name : "Blushing Harmony",
    price : 54,
    description : "Immerse yourself in the ethereal beauty of our 'Blushing Harmony'bouquet, where the gentle whispers of pink and the pristine elegance of white roses converge in a symphony of natural grace. Expertly curated by our artisans, each bloom is meticulously chosen for its freshness and allure, ensuring a captivating display that speaks volumes without words. The delicate blush of pink roses symbolizes admiration and sweetness, while the purity of white roses evokes innocence and new beginnings. Together, they create a harmonious blend that transcends any occasion, whether it's a celebration of love, a token of appreciation, or a moment of solace.",
    quantity : 7,
    image: blushing,
    careTips:"To maintain the beauty of your flower bouquet, start by trimming the stems at an angle and placing them in a clean vase filled with room temperature water. Remember to change the water every 2-3 days, trimming the stems each time to ensure optimal hydration. Keep the bouquet away from direct sunlight, heat sources, and drafts, and consider misting the flowers if the air is dry. Removing any wilted flowers promptly and avoiding submerging leaves in the water will help prevent bacterial growth. With these simple care steps, your flower bouquet will stay fresh and vibrant for longer, brightening up your space and lifting your spirits.",
    category: "flower"
  }
  
]